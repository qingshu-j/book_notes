...
SPErr PlayeventCopy(/*your parameters go here*/void);
...
SPErr PlayeventCopy(/*your parameters go here*/void)
{
	...
	PIActionDescriptor desc0000000000000550 = NULL;

	error = sPSActionDescriptor->Make(&desc0000000000000550);
	if (error) goto returnError;

	error = sPSActionControl->StringIDToTypeID("copyHint", &runtimeKeyID);
	if (error) goto returnError;

	// Unicode String as UTF8: layers
	error = sPSActionDescriptor->PutString(desc0000000000000550, runtimeKeyID, "layers");
	if (error) goto returnError;

	error = sPSActionControl->Play(&result, eventCopy, desc0000000000000550, plugInDialogSilent);
	if (error) goto returnError;

returnError:
	if (result != NULL) sPSActionDescriptor->Free(result);
	if (desc0000000000000550 != NULL) sPSActionDescriptor->Free(desc0000000000000550);
	return error;
}
...
SPErr PlayeventMake(/*your parameters go here*/void)
{
	...
	// Unicode String as UTF8: ç»?1
	error = sPSActionDescriptor->PutString(desc0000000000000ce0, keyName, "×é 1");
	if (error) goto returnError;
